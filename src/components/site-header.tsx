    ...
    <header className="bg-primary text-white py-4 px-6 md:px-10 flex items-center justify-between h-20 shadow-lg z-50 sticky top-0" suppressHydrationWarning>
        ...
        <Link href="/" className="relative w-14 h-14 flex items-center justify-center bg-white rounded-full shadow-md overflow-hidden transition-all hover:scale-105 group">
          {logo && (
            <Image 
              ...
              alt="NEU Logo" 
              fill
              className="object-contain p-1.5 transition-transform group-hover:rotate-6"
      ...
      <div className="flex items-center gap-4">
        {isMounted && (
          <>
            {!isHome && (
              <Button asChild variant="outline" className="bg-white/10 border-white/20 text-white hover:bg-white/20 rounded-full h-9 px-4 font-bold uppercase text-[10px] tracking-widest gap-2 shadow-sm transition-all" suppressHydrationWarning>
                <Link href="/">
